-- Bhakti — add the missing AUTH-APP tables to the live project.
--
-- WHY: project ref `ygpcbvmrcvbgjrhvvggf` already has the CMS/content tables
-- (temples, quotes, gods, mantras, festivals, panchang, jaap_sessions) but is
-- MISSING the auth-app tables the Next.js app needs: profiles, favorites,
-- jap_logs. Login works without these (users live in auth.users), but profile
-- / favourites / jap-count persistence silently no-ops until this runs.
--
-- HOW TO RUN: Supabase Dashboard → SQL Editor → New query → paste ALL of this
-- → Run. Safe to re-run (idempotent). Confirm the project ref is
-- `ygpcbvmrcvbgjrhvvggf` first.
--
-- NOTE: passwords are NOT here and never will be — Supabase stores those as a
-- bcrypt hash in auth.users, hidden and inaccessible. This table only holds the
-- profile fields (name, sankalp, phone, city, ...).
...
-- Backfill columns for anyone who created an earlier, smaller version.
...
-- ===========================================================================
-- 3. jap_logs  (1 row per user per day)  +  bump_jap RPC
-- ===========================================================================
