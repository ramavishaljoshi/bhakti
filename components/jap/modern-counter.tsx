...
import "./modern-counter.css";
import PhysicsMala from "./physics-mala";
...
const NAV_LINKS = [
  { label: "Counter", href: "/jap", active: true },
...
export default function ModernCounter({ mantraName }: { mantraName?: string }) {
  // Bridge each chant into the app-wide jap tracker (daily reset + per-user
  // persistence) so the home progress card and stats stay in sync.
  const { addJap } = useJap();
  const [settings, setSettings] = useState<Settings>(
    mantraName ? { ...DEFAULTS, mantra: mantraName, deity: mantraName.split(" ")[0] || "None" } : DEFAULTS
  );
  ...
  useEffect(() => {
    ...
    const seeded = mantraName
      ? { mantra: mantraName, deity: mantraName.split(" ")[0] || "None" }
      : {};
    setSettings({ ...DEFAULTS, ...stored, ...seeded });
  ...
  const addCount = useCallback(() => {
    ...
    // Record this chant in the app-wide tracker (one bead = +1).
    addJap(1, settingsRef.current.mantra);
    setPulse((p) => p + 1);
    setCount((prev) => {
      const s = settingsRef.current;
      const next = prev + 1;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [running, elapsed, refreshDerived, addJap]);
